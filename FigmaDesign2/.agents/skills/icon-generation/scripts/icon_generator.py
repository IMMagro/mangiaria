...
def generate_icon(prompt: str, size: int = 512, output_path: str = None,
                  ...
                  bg_removal: str = "builtin", input_image: str = None) -> dict:
    ...
    try:
        ...
        if input_image:
            ...
            if not Path(input_image).exists():
                return {"error": f"Input image not found: {input_image}"}
            shutil.copy2(input_image, temp_file)
        else:
...
Examples:
  ...
  python icon_generator.py -b '["home", "search", "profile"]' -r brand_icon.png
    ...
    parser.add_argument("--input", "-i", help="Use an existing image instead of generating one (skips API call)")
    ...
    # Validate input
    if not args.prompt and not args.batch and not args.input:
        parser.error("Either --prompt, --batch, or --input is required")
    ...
    if args.batch:
        ...
        for size in sizes:
            ...
            result = generate_icon(prompt, size, output_path, args.references, args.format, args.bg_removal, args.input)
