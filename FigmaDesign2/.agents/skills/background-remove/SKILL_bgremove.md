...
**Single image:**
```bash
python3 .agents/skills/background-remove/scripts/background_remove.py \
  -i "/path/to/image.jpg" \
...
|-----------|-------|-------------|---------|
| `--input` | `-i` | Input image path(s) (can be multiple) | Required |
...
# Adjust path based on your script's location
skill_root = Path(__file__).resolve().parent.parent.parent
bg_remove_path = skill_root / "skills" / "background-remove" / "scripts"
...
*The script will automatically fall back to the built-in method or return an error depending on integration.*
...
- GPU acceleration - Install `rembg[gpu]` for faster processing on NVIDIA GPUs
